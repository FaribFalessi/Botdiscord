...
const eventos = [
    { nombre: 'ROBO A VEHÍCULO', dias: [2, 5], horarios: ['02:30', '13:00', '15:00', '17:00'], duracion: 2, recordatorio: true },
...
client.once('ready', async () => {
    ...
    eventos.forEach(evento => {
        evento.horarios.forEach(horario => {
            const [hora, minuto] = convertirHorarioArgentina(horario).split(':');
            cron.schedule(`${minuto} ${hora} * * ${evento.dias.join(',')}`, async () => {
                const embed = generarEmbed(evento);
                const mensaje = await canal.send({ content: `<@&${roleId}>`, embeds: [embed] });
                await mensaje.react(reactionEmoji);

                // Iniciar recordatorios cada 1 hora
                if (evento.recordatorio) {
                    const intervalId = setInterval(async () => {
                        if (!activeReminders.has(mensaje.id)) return clearInterval(intervalId);
                        await canal.send({ content: `⏰ Recordatorio: **${evento.nombre}** sigue activo. ¡No lo olvides!`, embeds: [embed] });
                    }, 3600000);
                    activeReminders.set(mensaje.id, intervalId);
                }
            });
        });
    });
...
client.on('messageReactionAdd', async (reaction, user) => {
    ...
    if (reaction.emoji.name === reactionEmoji) {
        ...
        if (activeReminders.has(reaction.message.id)) {
            clearInterval(activeReminders.get(reaction.message.id));
            activeReminders.delete(reaction.message.id);
        }
    }
});
